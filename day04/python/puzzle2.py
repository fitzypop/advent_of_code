...
def main() -> int:
    ...
    result = -1
    won_cards = set()
    for draw, (i, masked_board) in product(
        draws, enumerate([np.ma.masked_array(board) for board in boards])
    ):
        # np special operates
        ...
        if np.any(masked_board.mask.sum(0) == 5) or np.any(
            ...
            # check for the last board by comparing lengths of lists
            if i not in won_cards and len(won_cards) == len(boards) - 1:
                result = masked_board.sum() * draw
                break
            # There was a bingo, and its not the last card to win
            # add to the list of won cards
            won_cards.add(i)

    print(f"Answer for day 4/puzzle 2 : {result}")
    return 0
